...
- **Amigos con Supabase — HECHO (2026-08-24):** Alejandro **cambió de idea** sobre la nube (antes cancelada el 2026-06-29) y pidió amigos con trayectoria en vivo. Implementado: `src/lib/cloud.ts` (config por `VITE_SUPABASE_URL`/`VITE_SUPABASE_ANON_KEY` o pegada en la app), pantalla `/friends` y `autoSync()` al arrancar. **Flujo sin fricción (rediseñado el mismo día):** entrar es `signInAnonymous()` (cero formularios, usa el nombre del perfil local) y se invita con un **enlace** `#/friends?add=<código>` que al abrirse crea la amistad ya aceptada (`add_friend`); `regenerate_friend_code()` invalida enlaces repartidos; quien quiera, luego pone correo+contraseña con `linkAccount()` para no perder la cuenta. Esquema + RLS en `docs/supabase/schema.sql`, **verificado con Postgres local** (solo tus amigos ven tus resúmenes, nadie escribe en los de otro, el código no se puede falsear y al regenerarlo los enlaces viejos mueren). **CONECTADO Y EN PRODUCCIÓN (2026-08-24).** Proyecto Supabase **`nutripiki`**, ref `vunhcilrjcwxuqqqzuyg`, región `eu-west-3` (París), org `piki2066's Org`. URL + clave *publishable* compiladas en `.env` (sí está en el repo: es la clave pública, protegida por RLS). Login anónimo activado y confirmación de correo desactivada vía `supabase/config.toml` + `supabase config push`. Probado en vivo con dos navegadores: uno invita, el otro abre el enlace y se ven al momento. Si `/friends` no estuviera configurado, enseña los pasos y un formulario para pegar credenciales (sigue sirviendo para otra instalación).
...
- **Supabase gratis (comprobado en supabase.com/pricing, 2026-08-24):** entrar con la cuenta de **GitHub** (no hace falta cuenta nueva ni tarjeta). Límites: 500 MB de base, 50.000 usuarios activos/mes, 5 GB de tráfico, peticiones ilimitadas, 2 proyectos activos. **Los proyectos gratuitos se pausan tras 1 semana sin actividad** → por eso existe `.github/workflows/keep-supabase-alive.yml` (cada 3 días llama a la función `ping()`; necesita los secretos `SUPABASE_URL` y `SUPABASE_ANON_KEY` en el repo; sin ellos no hace nada).
...
## Supabase (cuenta y operaciones)

- **CLI:** no está instalado globalmente; usar **`npx supabase@latest …`**. La sesión de Alejandro vive en el **llavero de macOS** ("Supabase CLI") y funciona sin volver a iniciar sesión.
- **Único proyecto:** `nutripiki` / `vunhcilrjcwxuqqqzuyg`. (El 2026-08-24 Alejandro pidió borrar los otros dos —`nr-boutique` y el vacío por defecto— y se borraron **sin copia**, avisándole antes de que era irreversible.)
- **Reaplicar el esquema:** está versionado en `supabase/migrations/`; `npx supabase@latest db push --password "$(cat ~/.nutripiki-supabase-db-password.txt)"`.
- **Contraseña de la base:** `~/.nutripiki-supabase-db-password.txt` (fuera del repo, permisos 600). **Nunca** meter en el repo esa contraseña ni la clave `service_role` / `sb_secret_`.
- **Anti-pausa:** `.github/workflows/keep-supabase-alive.yml` cada 3 días; secretos `SUPABASE_URL` y `SUPABASE_ANON_KEY` ya puestos en el repo.
